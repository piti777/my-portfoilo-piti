...
const About = ({isDarkMode}) => {
  return (
    <motion.div id="about" className="w-full px-[12%] px-[12%] py-10 scroll-mt-20"
    initial={{opacity:0}}
        ...
        transition={{duration:0.8,delay:1}}
            ...
            {infoList.map(({ icon, iconDark, title, description }, index) => (
              ...
              whileHover={{scale:1.05}}
                className="border-[0.5px] border-gray-400 
                            rounded-lx p-6 cursor-pointer hover:bg-lightHover
          ...
          className="flex items-center gap-3 sm:gap-5">
            {toolsData.map((tool, index) => (
              ...
              cursor-poiner hover:translate-y-1 duration-500"
                key={index}
              >
                <Image
                  src={tool}
